...
OUTPUT_PREFIX = "injection_rate_sweep"
RAW_CSV_OUTPUT_PATH = f"{OUTPUT_PREFIX}_raw.csv"
AGGREGATED_CSV_OUTPUT_PATH = f"{OUTPUT_PREFIX}_aggregated.csv"
SUMMARY_REPORT_OUTPUT_PATH = f"{OUTPUT_PREFIX}_summary.txt"
...
class InterposerMesh:
    ...
    def _print_drain_debug(self, cycle: int, message: str, force: bool = False) -> None:
        if force and (DRAIN_DEBUG_PRINTS and (cycle % max(1, DRAIN_DEBUG_PRINT_PERIOD) == 1)):
    ...
    def _perform_drain_hop(self, cycle: int) -> int:
        """
        ...
        self._print_drain_debug(
            cycle,
            f"drain_hop: candidates={len(candidates)} forwarded={len(movable)} blocked={blocked}",
        )
    ...
    def step(self, cycle: int) -> int:
        ...
        if self._full_drain_active():
            hop_moves = self._perform_drain_hop(cycle)
            moves += hop_moves
        ...
        if drain_this_cycle and self._drain_mode == DRAIN_MODE_DRAIN:
            moves += self._perform_drain_hop(cycle)
...
def scenario_display_name(label: str) -> str:
    return SCENARIO_DISPLAY.get(label, label)
...
def _flatten_type_metrics(per_type: Dict[str, Dict[str, float]]) -> Dict[str, float]:
    flat: Dict[str, float] = {}
    type_map = {
        CHIPLET_CPU: "cpu",
        CHIPLET_GPU: "gpu",
    }
    for chiplet_type, prefix in type_map.items():
        metrics = per_type.get(chiplet_type, {})
        flat[f"{prefix}_attempted"] = float(metrics.get("attempted_injections", 0))
        flat[f"{prefix}_injected"] = float(metrics.get("injected", 0))
        flat[f"{prefix}_failed"] = float(metrics.get("failed_injections", 0))
        flat[f"{prefix}_accept_ratio"] = float(metrics.get("acceptance_rate", 0.0))
        flat[f"{prefix}_delivered"] = float(metrics.get("delivered", 0))
        flat[f"{prefix}_throughput"] = float(metrics.get("throughput", 0.0))
        flat[f"{prefix}_avg_latency"] = float(metrics.get("avg_latency", 0.0))
    return flat


def _build_raw_sweep_row(
    scenario: str,
    ...
    row: Dict[str, Any] = {
        ...
        "seed": seed_value,
        "attempted": int(stats.get("attempted_injections", 0)),
        "injected": int(stats.get("injected", 0)),
        "delivered": int(stats.get("delivered", 0)),
        "failed": int(stats.get("failed_injections", 0)),
        "throughput": float(stats.get("throughput", 0.0)),
        "avg_latency": float(stats.get("avg_latency", 0.0)),
        "avg_hops": float(stats.get("avg_hops", 0.0)),
    }
    row.update(_flatten_type_metrics(stats.get("per_type", {})))
    return row


def aggregate_injection_sweep_rows(raw_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    grouped: Dict[Tuple[str, float], List[Dict[str, Any]]] = defaultdict(list)
    for row in raw_rows:
        key = (str(row["scenario"]), float(row["injection_rate"]))
    ...
    for (scenario, injection_rate), rows in sorted(grouped.items()):
        ...
        for metric in overall_metrics:
            ...
            agg_row[f"{metric}_mean"] = round(mean, 6)
            agg_row[f"{metric}_std"] = round(std, 6)

        for prefix in ("cpu", "gpu"):
            for metric in type_metrics:
                values = [float(row[f"{prefix}_{metric}"]) for row in rows]
                mean, std = _mean_std(values)
                agg_row[f"{prefix}_{metric}_mean"] = round(mean, 6)
                agg_row[f"{prefix}_{metric}_std"] = round(std, 6)
...
def write_raw_injection_sweep_csv(
    raw_rows: List[Dict[str, Any]],
    out_file: str = RAW_CSV_OUTPUT_PATH,
) -> None:
    fieldnames = [
        "scenario",
        "injection_rate",
...
def write_aggregated_injection_sweep_csv(
    aggregated_rows: List[Dict[str, Any]],
    out_file: str = AGGREGATED_CSV_OUTPUT_PATH,
) -> None:
    fieldnames = [
        "scenario",
        "injection_rate",
        ...
        "cpu_failed_mean",
        "cpu_failed_std",
        "cpu_accept_ratio_mean",
        "cpu_accept_ratio_std",
        "cpu_delivered_mean",
        "cpu_delivered_std",
        "cpu_throughput_mean",
        "cpu_throughput_std",
        "cpu_avg_latency_mean",
        "cpu_avg_latency_std",
        "gpu_attempted_mean",
        "gpu_attempted_std",
        "gpu_injected_mean",
        ...
        "gpu_failed_mean",
        "gpu_failed_std",
        "gpu_accept_ratio_mean",
        "gpu_accept_ratio_std",
        "gpu_delivered_mean",
        "gpu_delivered_std",
        "gpu_throughput_mean",
        "gpu_throughput_std",
        "gpu_avg_latency_mean",
        "gpu_avg_latency_std",
    ]
    _ensure_parent_dir(out_file)
    with open(out_file, "w", newline="") as fh:
...
def write_injection_sweep_summary_report(
    sweep: Dict[str, Dict[float, Dict[str, Any]]],
    out_file: str = SUMMARY_REPORT_OUTPUT_PATH,
) -> None:
    lines: List[str] = []
    lines.append("Injection Sweep Summary")
    lines.append("=" * 80)
    lines.append("")

    for label in SCENARIO_LABELS:
        scenario = sweep.get(label, {})
        if not scenario:
            continue
        rates = sorted(scenario.keys())
        peak_rate = max(rates, key=lambda rate: float(scenario[rate]["throughput"]))
        peak_metrics = scenario[peak_rate]
        final_rate = rates[-1]
        final_metrics = scenario[final_rate]

        lines.append(f"[{label}] {scenario_display_name(label)}")
        lines.append(
            f"  Peak throughput: {float(peak_metrics['throughput']):.6f} "
            f"at injection rate {peak_rate:.3f}"
        )
        lines.append(f"  Highest injection rate tested: {final_rate:.3f}")
        lines.append(f"  CPU/GPU breakdown at peak-throughput rate ({peak_rate:.3f}):")
        for chiplet_type in (CHIPLET_CPU, CHIPLET_GPU):
            type_metrics = peak_metrics.get("per_type", {}).get(chiplet_type, {})
            lines.append(
                f"    {chiplet_type}: attempted={int(type_metrics.get('attempted_injections', 0))}  "
                f"injected={int(type_metrics.get('injected', 0))}  "
                f"failed={int(type_metrics.get('failed_injections', 0))}  "
                f"accept_ratio={float(type_metrics.get('acceptance_rate', 0.0)):.4f}  "
                f"delivered={int(type_metrics.get('delivered', 0))}  "
                f"throughput={float(type_metrics.get('throughput', 0.0)):.6f}  "
                f"avg_latency={float(type_metrics.get('avg_latency', 0.0)):.2f}"
            )
        lines.append(f"  CPU/GPU breakdown at highest injection rate ({final_rate:.3f}):")
        for chiplet_type in (CHIPLET_CPU, CHIPLET_GPU):
            type_metrics = final_metrics.get("per_type", {}).get(chiplet_type, {})
            lines.append(
                f"    {chiplet_type}: attempted={int(type_metrics.get('attempted_injections', 0))}  "
                f"injected={int(type_metrics.get('injected', 0))}  "
                f"failed={int(type_metrics.get('failed_injections', 0))}  "
...
def sweep_injection_rates(
    ...
    raw_csv_path:       Optional[str]    = RAW_CSV_OUTPUT_PATH,
    aggregated_csv_path: Optional[str]   = AGGREGATED_CSV_OUTPUT_PATH,
    summary_report_path: Optional[str]   = SUMMARY_REPORT_OUTPUT_PATH,
) -> Dict[str, Dict[float, Dict[str, Any]]]:
    """
    Sweep CPU INJECTION_RATE and collect metrics per scenario.
    ...
    drain_period_c     : Kept for compatibility with earlier experiments.
                         The current active path uses the fixed ``drain_period``
                         value directly.
    drain_period_alpha : Exponent for the inverse power law (default 2.0).

    Returns
    -------
    sweep[label][rate] contains averaged metrics for plotting/reporting.
    """
    if chiplet_specs is None:
        chiplet_specs = build_image_layout()
    ...
    for idx, rate in enumerate(rates):
        ...
        per_type_accum: Dict[str, Dict[str, Dict[str, float]]] = {
            label: {} for label in SCENARIO_LABELS
        }
        for seed_offset in range(num_seeds):
            seed_value = seed + seed_offset * 1000
            ...
            for label in SCENARIO_LABELS:
                ...
                raw_rows.append(_build_raw_sweep_row(label, rate, seed_value, stats))
                accum[label]["attempted_injections"] += float(stats["attempted_injections"])
                accum[label]["injected"] += float(stats["injected"])
                accum[label]["delivered"] += float(stats["delivered"])
                accum[label]["failed_injections"] += float(stats["failed_injections"])
                accum[label]["throughput"] += float(stats["throughput"])
                accum[label]["avg_latency"] += float(stats["avg_latency"])
                accum[label]["avg_hops"] += float(stats["avg_hops"])

                for chiplet_type, type_stats in stats.get("per_type", {}).items():
                    type_accum = per_type_accum[label].setdefault(
                        chiplet_type,
                        {
                            "attempted_injections": 0.0,
                            "injected": 0.0,
                    ...
                    type_accum["attempted_injections"] += float(
                        type_stats.get("attempted_injections", 0)
                    )
                    type_accum["injected"] += float(type_stats.get("injected", 0))
                    type_accum["failed_injections"] += float(
                        type_stats.get("failed_injections", 0)
                    )
                    type_accum["delivered"] += float(type_stats.get("delivered", 0))
                    type_accum["throughput"] += float(type_stats.get("throughput", 0.0))
                    type_accum["avg_latency"] += float(type_stats.get("avg_latency", 0.0))

        for label in SCENARIO_LABELS:
            avg_attempted = accum[label]["attempted_injections"] / num_seeds
            avg_injected = accum[label]["injected"] / num_seeds
            avg_delivered = accum[label]["delivered"] / num_seeds
            avg_failed = accum[label]["failed_injections"] / num_seeds
            avg_throughput = accum[label]["throughput"] / num_seeds
            acceptance_rate = avg_injected / avg_attempted if avg_attempted else 0.0

            averaged_per_type: Dict[str, Dict[str, float]] = {}
            for chiplet_type, type_accum in per_type_accum[label].items():
                type_attempted = type_accum["attempted_injections"] / num_seeds
                type_injected = type_accum["injected"] / num_seeds
                type_failed = type_accum["failed_injections"] / num_seeds
                type_delivered = type_accum["delivered"] / num_seeds
                type_acceptance = type_injected / type_attempted if type_attempted else 0.0
                averaged_per_type[chiplet_type] = {
                    "attempted_injections": int(round(type_attempted)),
                    "injected": int(round(type_injected)),
                    "failed_injections": int(round(type_failed)),
                    "acceptance_rate": round(type_acceptance, 4),
                    "delivered": int(round(type_delivered)),
                    "throughput": round(type_accum["throughput"] / num_seeds, 6),
                    "avg_latency": type_accum["avg_latency"] / num_seeds,
                }

            sweep[label][rate] = {
                "attempted_injections": int(round(avg_attempted)),
                "acceptance_rate": round(acceptance_rate, 4),
                "injected": int(round(avg_injected)),
                "delivered": int(round(avg_delivered)),
                "failed_injections": int(round(avg_failed)),
                "throughput": round(avg_throughput, 6),
                "avg_latency": accum[label]["avg_latency"] / num_seeds,
                "avg_hops": accum[label]["avg_hops"] / num_seeds,
                "per_type": averaged_per_type,
            }

    aggregated_rows = aggregate_injection_sweep_rows(raw_rows)
    # if raw_csv_path is not None:
        # write_raw_injection_sweep_csv(raw_rows, out_file=raw_csv_path)
    if aggregated_csv_path is not None:
        write_aggregated_injection_sweep_csv(aggregated_rows, out_file=aggregated_csv_path)
    if summary_report_path is not None:
        write_injection_sweep_summary_report(sweep, out_file=summary_report_path)
